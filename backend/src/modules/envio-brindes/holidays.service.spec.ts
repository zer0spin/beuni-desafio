...
describe('HolidaysService', () => {
  ...
  describe('isHoliday', () => {
    it('should return true for fixed holidays', () => {
      ...
      expect(service.isHoliday(new Date('2025-01-01'))).toBe(true);

      // Tiradentes
      expect(service.isHoliday(new Date('2025-04-21'))).toBe(true);

      // Natal
      expect(service.isHoliday(new Date('2025-12-25'))).toBe(true);
    });

    it('should return true for movable holidays', () => {
      // Carnaval 2025
      expect(service.isHoliday(new Date('2025-03-03'))).toBe(true);

      // Sexta-feira Santa 2025
      expect(service.isHoliday(new Date('2025-04-18'))).toBe(true);

      // Corpus Christi 2025
      expect(service.isHoliday(new Date('2025-06-19'))).toBe(true);
    });

    it('should return false for non-holidays', () => {
      expect(service.isHoliday(new Date('2025-01-02'))).toBe(false);
      expect(service.isHoliday(new Date('2025-06-15'))).toBe(false);
      expect(service.isHoliday(new Date('2025-08-10'))).toBe(false);
    });

    it('should work correctly for different years', () => {
      // Carnaval varia por ano
      expect(service.isHoliday(new Date('2024-02-12'))).toBe(true); // Carnaval 2024
      expect(service.isHoliday(new Date('2025-03-03'))).toBe(true); // Carnaval 2025
      expect(service.isHoliday(new Date('2026-02-16'))).toBe(true); // Carnaval 2026
    });
  });

  describe('getHolidayName', () => {
    it('should return correct name for fixed holidays', () => {
      expect(service.getHolidayName(new Date('2025-01-01'))).toBe('Confraternização Universal');
      expect(service.getHolidayName(new Date('2025-04-21'))).toBe('Tiradentes');
      expect(service.getHolidayName(new Date('2025-12-25'))).toBe('Natal');
    });

    it('should return correct name for movable holidays', () => {
      expect(service.getHolidayName(new Date('2025-03-03'))).toBe('Carnaval');
      expect(service.getHolidayName(new Date('2025-04-18'))).toBe('Sexta-feira Santa');
      expect(service.getHolidayName(new Date('2025-06-19'))).toBe('Corpus Christi');
    });

    it('should return null for non-holidays', () => {
      expect(service.getHolidayName(new Date('2025-01-02'))).toBeNull();
      expect(service.getHolidayName(new Date('2025-06-15'))).toBeNull();
    });
  });

  describe('getUpcomingHolidays', () => {
    it('should return next 5 holidays from a given date', () => {
      const upcomingHolidays = service.getUpcomingHolidays(new Date('2025-01-15'), 5);
    ...
    it('should include holidays from next year if needed', () => {
      const upcomingHolidays = service.getUpcomingHolidays(new Date('2025-12-20'), 3);
