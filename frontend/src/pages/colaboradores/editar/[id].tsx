...
export default function EditarColaboradorPage() {
  ...
  const loadColaborador = async (colaboradorId: string) => {
    try {
      ...
      // Set CepData based on current address
      setCepData({
        logradouro: data.endereco.logradouro,
        bairro: data.endereco.bairro,
        cidade: data.endereco.cidade,
  ...
  const handleCepChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } else if (cep.length === 0 && colaborador) {
      // Reset to original address data when CEP is empty
      setCepData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/colaboradores/${id}`, updateData);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-yellow-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-lg font-medium text-gray-900">Colaborador não encontrado</h2>
          <Link href="/colaboradores" className="text-orange-600 hover:text-orange-700 mt-2 inline-block">
          ...
          <div className="flex items-center justify-between py-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Atualize os dados de {colaborador.nome_completo}</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <button
                ...
                disabled={loading}
                className="inline-flex items-center px-4 py-2 text-red-600 border border-red-300 rounded-lg hover:bg-red-50 font-medium transition-colors disabled:opacity-50"
              >
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-orange-100 p-6">
            <div className="flex items-center space-x-2 mb-6">
              ...
              <span className="text-sm text-gray-500">(deixe em branco para manter o endereço atual)</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-4">
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  disabled={!!cepData && !!formData.endereco.cep}
            ...
            {cepData && (
              <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                <h3 className="text-sm font-medium text-green-800 mb-2">
                  {formData.endereco.cep ? 'Novo endereço:' : 'Endereço atual:'}
                </h3>
